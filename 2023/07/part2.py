...
class Hand(object):
    def __init__(self, cards):
        ...
        self.type = max([self.get_type(J=card) for card in 'J23456789TQKA'])

    def get_type(self, J):
        cards = self.cards.replace('J', J)
        unique = set(cards)  # the unique cards we have
        occurrences = sorted([cards.count(u) for u in unique], reverse=True)  # the occurrence of the most common, second most common, etc, card
        if len(unique) == 1:
    ...
    def __gt__(self, other):
        if self.type == other.type:
            val_map = {char:val for (val, char) in enumerate('J23456789TQKA')}  ## new order in part 2
